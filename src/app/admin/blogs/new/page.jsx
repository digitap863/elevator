  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        alert(json.error || 'Failed to create blog post');
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl mx-auto animate-fade-in pb-16">
      ...
      <div className="sticky top-16 z-30 bg-white/95 backdrop-blur-md border border-gray-200/80 p-3.5 md:p-4 rounded-2xl shadow-xs flex flex-wrap items-center justify-between gap-4 transition-all">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden sm:flex items-center gap-2 min-w-0">
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Creating:</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 bg-white border border-gray-150 rounded-3xl p-6 md:p-8 shadow-xs space-y-6">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={handleTextChange}
              rows="3"
              placeholder="Summarize your article in 2-3 sentences. Displays in card listing grids..."
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-slate-900/5 focus:border-slate-800 transition-all text-sm font-satoshi"
            />
          </div>
